...
public class Primitives {
	...
	private var readFileData:String;
	...
	public function addPrimsTo(primTable:Dictionary):void {
		...
		primTable["deleteAllClones"]    = primDeleteAllClones;
		primTable["whenCloned"]			= interp.primNoop;
		primTable["numberOfClones"]     = primNumberOfClones;

		// files
		//primTable["readFile:"]          = primReadFile;
		//primTable["write:toFile:"]      = primWriteToFile;
		primTable["save:toFile:"]        = primSaveToFile;
	...
	private function primDeleteAllClones(b:Block):void {
		app.stagePane.deleteClones();
	}

	private function primNumberOfClones(b:Block):Number {
		return app.runtime.cloneCount;
	}

	private function primReadFile(b:Block):String {
		var fileName:String = interp.arg(b, 0);
		var textURL:URLRequest = new URLRequest(fileName);
		var textLoader:URLLoader = new URLLoader();
		var fileData:String;

		textLoader.addEventListener(Event.COMPLETE, completeReadFileHandler);
	...
	private function completeReadFileHandler(e:Event):void { readFileData=e.target.data; }

	private function primWriteToFile(b:Block):void {
		//Will be implemented later
	}

	private function primSaveToFile(b:Block):void {
		var textToSave:String = interp.arg(b, 0);
		var fileName:String = interp.arg(b, 1);
		var file:FileReference = new FileReference;
		file.save(textToSave, fileName);
	}
